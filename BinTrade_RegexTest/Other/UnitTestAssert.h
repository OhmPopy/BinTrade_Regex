...
#define EQUALS_MESSAGE_WINRT(expected, actual, message)     reinterpret_cast<const unsigned short*>(GetAssertMessage(true, ToWideString(expected), ToWideString(actual), NULL == message ? NULL : message->Data()).c_str())
#define NOT_EQUALS_MESSAGE_WINRT(expected, actual, message) reinterpret_cast<const unsigned short*>(GetAssertMessage(false, ToWideString(expected), ToWideString(actual), NULL == message ? NULL : message->Data()).c_str())
#endif
...
template <typename Q>
static std::wstring ToString(const Q& q) { static_assert(false, "Test writer must define specialization of ToString<const Q& q> for your class \"__FUNCSIG__\"."); }

template <typename Q>
static std::wstring ToString(const Q* q) { static_assert(false, "Test writer must define specialization of ToString<const Q* q> for your class \"__FUNCSIG__\"."); }

template <typename Q>
static std::wstring ToString(Q* q) { static_assert(false, "Test writer must define specialization of ToString<Q* q> for your class \"__FUNCSIG__\"."); }


// ToString() template specialization for basic types.
template <>
static std::wstring ToString<bool>(const bool& t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<int>(const int& t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<long>(const long& t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<short>(const short& t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<char>(const char& t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<wchar_t>(const wchar_t& t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<signed char>(const signed char& t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<unsigned int>(const unsigned int& t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<unsigned long>(const unsigned long& t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<unsigned long long>(const unsigned long long& t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<unsigned char>(const unsigned char& t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<std::string>(const std::string& t)
{
	RETURN_WIDE_STRING(t.c_str());
}

template <>
static std::wstring ToString<std::wstring>(const std::wstring& t) { return t; }

template <>
static std::wstring ToString<double>(const double& t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<float>(const float& t)
{
	RETURN_WIDE_STRING(t);
}
...
template <>
static std::wstring ToString<void>(const void* t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<bool>(bool* t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<int>(int* t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<long>(long* t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<short>(short* t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<signed char>(signed char* t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<unsigned int>(unsigned int* t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<unsigned long>(unsigned long* t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<unsigned long long>(unsigned long long* t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<unsigned char>(unsigned char* t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<char>(char* t)
{
	if (NULL == t) return std::wstring(L"(NULL)");
	RETURN_WIDE_STRING(t);
}
...
template <>
static std::wstring ToString<wchar_t>(wchar_t* t)
{
	if (NULL == t) return std::wstring(L"(NULL)");
	RETURN_WIDE_STRING(t);
}
...
template <>
static std::wstring ToString<double>(double* t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<float>(float* t)
{
	RETURN_WIDE_STRING(t);
}

template <>
static std::wstring ToString<void>(void* t)
{
	RETURN_WIDE_STRING(t);
}

#define AssertThrows( e,f) {\
    try {\
        f;\
    }\
    catch (e se) {\
...
class Assert
{
	static long long totalPass;
	static long long totalFaild;
public:
	static long long GetTotalPass();

	static long long GetTotalFaild();


	// Verify that two objects are equal.
	template <typename T>
	static void AreEqual(const T& expected, const T& actual, const wchar_t* message = NULL, const __LineInfo* pLineInfo = NULL)
	{
	...
	template <typename T>
	static void AreSame(const T& expected, const T& actual, const wchar_t* message = NULL, const __LineInfo* pLineInfo = NULL)
	{
		FailOnCondition(&expected == &actual, EQUALS_MESSAGE(expected, actual, message), pLineInfo);
	}
	...
	template <typename T>
	static void AreNotEqual(const T& notExpected, const T& actual, const wchar_t* message = NULL, const __LineInfo* pLineInfo = NULL)
	{
	...
	template <typename T>
	static void AreNotSame(const T& notExpected, const T& actual, const wchar_t* message = NULL, const __LineInfo* pLineInfo = NULL)
	{
		FailOnCondition(!(&notExpected == &actual), NOT_EQUALS_MESSAGE(notExpected, actual, message), pLineInfo);
	}
	...
	template <typename T>
	static void IsNull(const T* actual, const wchar_t* message = NULL, const __LineInfo* pLineInfo = NULL)
	{
	...
	template <typename T>
	static void IsNotNull(const T* actual, const wchar_t* message = NULL, const __LineInfo* pLineInfo = NULL)
	{
		FailOnCondition(!(actual == NULL), reinterpret_cast<const unsigned short*>(message), pLineInfo);
	}
	...
	static void True(bool condition, const wchar_t* message = NULL, const __LineInfo* pLineInfo = NULL);

	static void WriteLineSpace();
	static void WriteLine(const wchar_t* message = NULL);
	...
	static void False(bool condition, const wchar_t* message = NULL, const __LineInfo* pLineInfo = NULL);

	static void ThrowsTrue();

	static void ThrowsFalse();
	...
	template<typename T> static void AreEqual(T^ expected, T^ actual, Platform::String^ message = NULL, const __LineInfo* pLineInfo = NULL)
	{
	...
	static void AreEqual(Platform::String^ expected, Platform::String^ actual, char* message, const __LineInfo* pLineInfo = NULL)
	{
		AreEqual(expected, actual, DEFAULT_IGNORECASE, ref new Platform::String(ToString(message).c_str()), pLineInfo);
	}

	// Platform::String^ string equality comparison.
	static void AreEqual(Platform::String^ expected, Platform::String^ actual, wchar_t* message, const __LineInfo* pLineInfo = NULL)
	{
		AreEqual(expected, actual, DEFAULT_IGNORECASE, ref new Platform::String(message), pLineInfo);
	}

	// Platform::String^ string equality comparison.
	static void AreEqual(Platform::String^ expected, Platform::String^ actual, Platform::String^ message, const __LineInfo* pLineInfo = NULL)
	{
		AreEqual(expected, actual, DEFAULT_IGNORECASE, message, pLineInfo);
	}

	// Platform::String^ string equality comparison.
	static void AreEqual(Platform::String^ expected, Platform::String^ actual, bool ignoreCase = false, Platform::String^ message = NULL, const __LineInfo* pLineInfo = NULL)
	{
	...
	template<typename T> static void AreSame(T% expected, T% actual, Platform::String^ message = NULL, const __LineInfo* pLineInfo = NULL)
	{
	...
	template<typename T> static void AreNotEqual(T^ notExpected, T^ actual, Platform::String^ message = NULL, const __LineInfo* pLineInfo = NULL)
	{
		bool condition = true;
		...
		else if (notExpected == NULL && actual == NULL)
		{
			condition = false;
		}
	...
	// Platform::String^ string equality comparison.
	static void AreNotEqual(Platform::String^ expected, Platform::String^ actual, char* message, const __LineInfo* pLineInfo = NULL)
	{
		AreNotEqual(expected, actual, DEFAULT_IGNORECASE, ref new Platform::String(ToString(message).c_str()), pLineInfo);
	}

	// Platform::String^ string equality comparison.
	static void AreNotEqual(Platform::String^ expected, Platform::String^ actual, wchar_t* message, const __LineInfo* pLineInfo = NULL)
	{
		AreNotEqual(expected, actual, DEFAULT_IGNORECASE, ref new Platform::String(message), pLineInfo);
	}

	// Platform::String^ string equality comparison.
	static void AreNotEqual(Platform::String^ expected, Platform::String^ actual, Platform::String^ message, const __LineInfo* pLineInfo = NULL)
	{
		AreNotEqual(expected, actual, DEFAULT_IGNORECASE, message, pLineInfo);
	}

	// Platform::String^ string AreNotEqual comparison:
	static void AreNotEqual(Platform::String^ notExpected, Platform::String^ actual, bool ignoreCase = false, Platform::String^ message = NULL, const __LineInfo* pLineInfo = NULL)
	{
	...
	template<typename T> static void AreNotSame(T% notExpected, T% actual, Platform::String^ message = NULL, const __LineInfo* pLineInfo = NULL)
	{
		FailOnCondition(!(%notExpected == %actual), NOT_EQUALS_MESSAGE_WINRT(%notExpected, %actual, message), pLineInfo);
	}

	// Check if a WinRT pointer is NULL.
	template<typename T> static void IsNull(T^ actual, Platform::String^ message = NULL, const __LineInfo* pLineInfo = NULL)
	{
		FailOnCondition(actual == NULL, UnboxString(message), pLineInfo);
	}

	// Check if a WinRT pointer is not NULL
	template<typename T> static void IsNotNull(T^ actual, Platform::String^ message = NULL, const __LineInfo* pLineInfo = NULL)
	{
		FailOnCondition(!(actual == NULL), UnboxString(message), pLineInfo);
	}
	...
	static void ExpectException(_FUNCTOR functor, const wchar_t* message = NULL, const __LineInfo* pLineInfo = NULL)
	{
		try
		{
			functor();
	...
	static void ExpectException(_RETURNTYPE (*func)(), const wchar_t* message = NULL, const __LineInfo* pLineInfo = NULL)
	{
...
private:
	...
	static void __stdcall FailOnCondition(bool condition, const unsigned short* message, const __LineInfo* pLineInfo);

	static bool __stdcall CppUnitStrCmpA(const char* str1, const char* str2, bool ignoreCase);

	static bool __stdcall CppUnitStrCmpW(const unsigned short* str1, const unsigned short* str2, bool ignoreCase);

	static void __stdcall FailImpl(const unsigned short* message, const __LineInfo* pLineInfo);

	static void __stdcall GetAssertMessage(bool equality, const unsigned short* expected, const unsigned short* actual, const unsigned short* userMessage, unsigned short* assertMessageBuffer, size_t bufferSize);
